...
import com.jess.arms.utils.utilcode.util.PhoneUtils;
...
public class ArmsUtils {
    ...
    public static void setListViewHeightBasedOnChildren(ListView listView) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            params.height = totalHeight + (listView.getDividerHeight() * (listAdapter.getCount() - 1))+ SizeUtils.dp2px(15);
            //listView.getDividerHeight()获取子项间分隔符占用的高度
            //params.height最后得到整个ListView完整显示需要的高度
        }
    ...
    public static void setListViewHeightBasedOnChildren(RecyclerView listView) {
        ...
        ViewGroup.LayoutParams params = listView.getLayoutParams();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            params.height = totalHeight + (listView.getMeasuredHeight() * (listAdapter.getItemCount() - 1));
            //listView.getDividerHeight()获取子项间分隔符占用的高度
            //params.height最后得到整个ListView完整显示需要的高度
        }else {
            params.height = totalHeight + (listView.getMeasuredHeight() * (listAdapter.getItemCount() - 1))+ SizeUtils.dp2px(15);
            //listView.getDividerHeight()获取子项间分隔符占用的高度
            //params.height最后得到整个ListView完整显示需要的高度
        }
     ...
     * @param time
     */
    public static void finishDelay(BaseActivity context, long time){
        Observable.just(1).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .delay(time, TimeUnit.MILLISECONDS).subscribe(new Observer<Integer>() {
            @Override
